...
7. Write a query that has, in the US region only, the total spend in usd for each advertiser_name and how many ads they ran. (Hint, you're going to have to join tables for this one). 
	```
	WITH TABLE_AD_WEEKLY_SPEND AS (
	SELECT 
  	COUNT(advertiser_name) numberAds,
  	advertiser_name
	FROM 
  	`bigquery-public-data.google_political_ads.advertiser_weekly_spend` 
	GROUP BY 
	...
	* 
	...
	SELECT A.advertiser_name, numberAds, B.regions FROM TABLE_AD_WEEKLY_SPEND  as A JOIN TABLE_AD_STATS  as B ON A.advertiser_name = B.advertiser_name
...
3. Write a query that, for every station_name, has the amount of trips that started there and the amount of trips that ended there. (Hint, use two temporary tables, one that counts the amount of starts, the other that counts the number of ends, and then join the two.) 
	...
	WITH TBL_START AS (
  	SELECT  
    	...
    	COUNT(start_station_name) as countStart
  	FROM 
    	`bigquery-public-data.new_york_citibike.citibike_trips` 
  	GROUP BY 
   	 station
	...
	TBL_END AS (
  	SELECT 
    	...
    	COUNT(end_station_name) as countEnd
  	FROM 
    	`bigquery-public-data.new_york_citibike.citibike_trips` 
  	GROUP BY 
    	station
	)
	SELECT A.station, countStart , B.countEnd  FROM TBL_START as A JOIN TBL_END as B ON A.station  = B.station 
	```
...
6. Copy the link and paste it right below this line. 
	* YOUR LINK:  __https://colab.research.google.com/drive/1BCjkw-FWis3NqHo7QfCGmpAqvsLVATSE?usp=sharing______________________________
